...
6. DEDICATED CARDIO DAY: when the user's goal is weight loss or general fitness AND they have a stated cardio preference AND they train 4+ days per week, you MAY make ONE of the days a cardio-only conditioning day. On such a day: set "focus" to something like "Cardio / conditioning", leave "exercises" as an empty array ([]), fill the "cardio" block with a meaningful session (20-40 min steady state or intervals using their preferred/available modality), and still give a short warmup and cooldown. Never make more than one cardio-only day per week, and never leave a STRENGTH day with an empty exercises array.
