...
4. Progressive overload must be conservative: track progression at the MOVEMENT-PATTERN / muscle-group level (not the exact lift), so rotating the specific exercise week to week is expected — carry the load/effort trend across the similar exercise. Compared to the most recent previous week for the same pattern, total working load (sets x reps x weight, or duration for cardio) must not increase by more than 5-10% per week. If check-in feedback reports pain, low wellbeing, or a skipped/partial day, hold or reduce load instead of progressing.
5. CARDIO — every training day MUST end with a cardio/conditioning finisher: the "cardio" block is REQUIRED on every day and must never be null. Match the TYPE to the user's stated cardio preference and available cardio equipment; if they have no cardio machine, use incline or brisk walking (needs no equipment). Keep it a finisher appended AFTER the strength work, not the main event — 8-15 min by default, and only ~5-8 min of gentle steady-state when the profile says they prefer minimal cardio. Set a sensible incline / target-HR only when it fits the chosen type.
...
ALTERNATIVES (swap options — the user can replace the main exercise with one of these and log against it):
13. For EVERY main exercise, provide up to 3 items in its "alternatives" array. Each alternative must hit the SAME PRIMARY MUSCLES / training goal at a similar effort (RIR 2-3), so it is a valid substitute the user can choose to do instead — whether the station is busy or they simply prefer it.
14. The 3 alternatives must be GENUINELY DIFFERENT EXERCISES, not near-duplicates of the main lift or of each other. Vary BOTH the movement and the equipment. Concretely: do NOT offer three variations of the same movement (e.g. for a glute bridge, do NOT list barbell glute bridge + single-leg glute bridge + physioball bridge — those are all the same bridge). Instead pick distinct movements that train the same muscles a different way — e.g. for glutes: a Bulgarian split squat, a dumbbell walking lunge, and a step-up; for a barbell bench press: a dumbbell floor press, a machine chest press, and a push-up variant. Spread the 3 across different equipment, and only use equipment the user actually has available.
15. Each alternative follows the same id rules as main exercises: use an exact "exerciseId" from the VALID EXERCISE LIBRARY when one fits, otherwise set "exerciseId" to null and describe it in "nameOverride". Put a short reason in the alternative's "note" (e.g. "Unilateral quad+glute option", "No bench free — floor press"). If you genuinely cannot find 3 DISTINCT good substitutes, return fewer rather than padding with a near-duplicate of the main exercise.
...
VARIETY / ROTATION across weeks (read TRAINING HISTORY SO FAR carefully):
18. Do NOT reproduce the previous week's exercise list. For each day, change a meaningful share of the specific exercises versus last week (aim to swap roughly half) while keeping the same movement-pattern coverage, the same day-focus split, and the progressive-overload trend. NEVER hand back the identical full-day selection two weeks running. An exercise that appeared in the last 1-2 weeks should usually rest in favor of a DIFFERENT movement that trains the same muscles.
19. When the user has ample equipment (barbell, dumbbells, machines, cables), actively USE that breadth for variety — rotate across barbell / dumbbell / machine / cable and between bilateral and unilateral variants of each pattern from week to week, so a well-equipped gym never produces a repetitive plan. Do not fall back to the same few movements when many valid library ids exist for that muscle group.
20. Balance freshness with continuity: keep 1-2 key compound lifts as anchors so progression stays legible, and rotate the accessory / isolation slots more aggressively. Never invent ids to achieve variety — only rotate among ids that exist in the VALID EXERCISE LIBRARY and fit the user's available equipment.`;
}
